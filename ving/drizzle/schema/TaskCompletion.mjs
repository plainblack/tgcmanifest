...
export const TaskCompletionTable = mysqlTable('taskcompletions',
    ...
    (table) => ([
        foreignKey({ name: "taskcompletions_order_3c1145bf_fk", columns: [table.orderId], foreignColumns: [OrderTable.id]}).onDelete("cascade").onUpdate("cascade"),
		foreignKey({ name: "taskcompletions_user_5481f188_fk", columns: [table.completedById], foreignColumns: [UserTable.id]}).onDelete("cascade").onUpdate("cascade")
    ])
);
